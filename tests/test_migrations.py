"""The migration chain, against databases that already exist.

A deployment that is one version behind is the normal case, not the exotic
one. This file exists because replacing the whole schema left the previous
revision id unresolvable, and `alembic upgrade head` refuses to move a
database whose recorded revision it cannot find:

    ERROR  Can't locate revision identified by '6741599e8157'

The API container runs `alembic upgrade head && uvicorn ...`, so that exits
non-zero and the service never starts. Reproduced at exit code 255.
"""
...
def _upgrade(db_path: Path) -> subprocess.CompletedProcess[str]:
    """Run exactly what the container runs at startup."""
...
def legacy_db(tmp_path: Path) -> Path:
    ...
    con.execute("INSERT INTO alembic_version VALUES (?)", (LEGACY_REVISION,))
...
def test_upgrading_a_legacy_database_destroys_nothing(legacy_db: Path) -> None:
    """Those old rows are real interview records. Tidying up is not worth
    deleting someone's data for."""
...
def test_a_fresh_database_gets_no_dead_legacy_tables(tmp_path: Path) -> None:
    """The legacy revision is a placeholder so its id resolves, not a
    recreation of tables nothing reads any more."""
    db = tmp_path / "fresh.db"
