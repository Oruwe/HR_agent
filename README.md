...
Built for **YC Fall 2026 × Moss: The Zero Latency Builder Sprint** — Track 1, Real-Time Voice and Conversational AI.

This repository is two things layered together:

1. **The voice orchestration core** (`app/agent`, `app/voice`, `app/storage`) —
...
2. **A full application built on top of it** — an **API Gateway**
...
seconds is not a slow assistant — it is a different kind of object, one that
...
The standard cascaded loop — wait for silence, transcribe, retrieve, generate,
synthesise — spends 1500–4000ms per turn. This repository is an argument that
...
- **redacts PII deterministically** before anything reaches a model, a vector
  store, a trace, or a disk — verified by 82 tests;
...
| **Turnaround** (commit → first audio byte) | **12.44 ms** | **12.58 ms** | 12.79 ms | 12.91 ms | 150 ms |
| Turnaround, sequential baseline | 43.30 ms | 43.57 ms | 43.67 ms | 43.72 ms | 150 ms |

**Speculative turn-taking removes a median of 30.9ms per turn — 71% of the
sequential cost.**
...
- **Measured:** the orchestration this repository controls — endpointing,
...
- **Not included:** real WAN round-trip time to a hosted LLM. From Bangalore to
  a hosted inference endpoint that is realistically 200–400ms of TTFT.
...
off the network is a millisecond you get to keep — which is precisely why Moss
...
managed service costs 15–40ms *before the index does any work*, so the 5ms
line silently becomes 50ms and the 150ms total is arithmetic that does not
survive contact with production.

Moss is a **search runtime, not a database**. It runs in-process — browser,
edge, device, or cloud — so a query is a function call rather than a network
...
**Corpus design.** The nine rubrics are indexed as **36 documents — one per
...
> - **Moss (YC F25)** — the retrieval runtime. Configured via `MOSS_PROJECT_ID` / `MOSS_PROJECT_KEY`.
> - **MOSS-Speech** — an open speech-to-speech model, used for synthesis. Configured via `HRTE_SPEECH_*`.
>
...
| **Total** | **150 ms** | commit → first audio byte |
...
The largest lever on perceived latency is not the model — it is the endpoint
...
candidate stops speaking
   │
   ├─ 40ms  ─→ SPECULATE       retrieval + generation start; still listening
   │
   ├─ 120ms ─→ TURN_COMMIT     tokens already buffered → speak immediately
   │
   └─ if speech resumes in between → draft discarded, costs nothing but compute
```
...
turnaround is 12.44ms rather than 43ms — and `test_speculation_is_what_buys_the_budget`
...
frame is simply invalid — no lock, no cancellation point, no race.
...
```
app/
├── config.py            latency budget, settings, capability flags
├── schemas/             candidate · 9 role rubrics · evaluation dossier
├── security/            PII scrubber · credential guard
├── storage/
│   ├── retrieval.py     ← Moss runtime (hot path) + embedded fallback
│   ├── qdrant_client.py cold archive for completed dossiers
│   └── embeddings.py    multi-probe hashed encoder + BM25 sparse
├── voice/               ring buffer · VAD state machine · synthesis · LiveKit ·
│                         stt_engine.py (Deepgram + offline mock)
├── agent/               orchestrator · prompts · interview flow · cognition
├── telemetry/           per-stage metrics · PII-gated tracing
├── db/                  Primary DB: SQLAlchemy models + Alembic migrations
└── api/                 API Gateway: FastAPI app, session/admin/health routes,
                          Session State store

frontend/
└── src/
    ├── api/client.ts        typed fetch wrapper over the API Gateway
    ├── hooks/                useActiveSession (candidate session persistence),
    │                         useMicRecorder (MediaRecorder wrapper)
    ├── pages/candidate/      Intake -> Interview -> Results
    └── pages/admin/          Dashboard (sessions + system status), session detail
```
...
| Voice Orchestrator | `app/agent/orchestrator.py` | Unmodified from the original voice-agent core — the API layer drives the same `speculate()` / `commit()` sequence the LiveKit worker and the offline demo already used. |
...
| Moss Retrieval | `app/storage/retrieval.py` | Unmodified — already on the hot path; the API layer just surfaces `retrieval_backend` in responses. |
...
| LangFuse Telemetry | `app/telemetry/langfuse_tracer.py` | Unmodified — already wired into the orchestrator; unaffected by this work. |
...
- **The Candidate App and Admin Dashboard are one Vite project with
  ...
  conceptually — in practice they're the same static bundle.
...
ordering on every run. Retrieval agrees independently on all nine — meaningful
...
3. A strong candidate who trips one gate becomes `HOLD_FOR_HUMAN_REVIEW` — the
...
`re.sub` cascade is wrong in two ways that bite — pass *N* matches inside pass
*N−1*'s output, and every substitution destroys the offset mapping that
...
produces the same digest and duplicate applications are detectable — without
anything reversible ever being stored. Keyed rather than plain, deliberately: a
10-digit phone number is only 10¹⁰ candidates, so an *unkeyed* digest of one is
...
guessable — the safe failure, and the one you notice.
...
by a failing test — the generic passport rule was matching the word `REDACTED`
...
credentials the agent runs fully offline — deterministic embeddings, in-process
...
| `DATABASE_URL` | Primary DB — empty = SQLite, or a Postgres URL |
...
specification and are validated by tests rather than by inspection —
...
276 tests, zero warnings (filterwarnings = ["error"])
 ...
 82  PII redaction           coverage · false positives · algebra · crypto · gates
...
gate, health/readiness/Prometheus format, ORM relationships, and — the one
that actually matters for "the migration works, not just exists" — running
...
SQLite by default — `make migrate` creates `./data/app.db` with no further
...
This is the verification this project's own testing requirements call for —
...
say so): interactive browser behavior — clicking through the Candidate App in
an actual browser, granting microphone permission, playing back synthesized
audio — because this environment has no browser automation tool available.
...
- **Health checks:** `/health` (liveness — process is up) and `/ready`
  (readiness — database is reachable) are separate on purpose; a transient
...
- **Logging:** structured via Python's standard `logging`, one line per
  ...
  synthesis failure) — see the `logger.warning(...)` calls throughout
...
- **`alembic upgrade head` fails with "table already exists"** — the app's
...
- **Candidate App shows "voice transcription isn't configured"** — expected
  with no `DEEPGRAM_API_KEY` set; type the answer instead. This is the
  documented offline-fallback behavior, not a bug.
- **CORS errors in the browser console** — set `HRTE_CORS_ORIGINS` to include
...
- **`/ready` returns 503** — the database is unreachable. Check
  `DATABASE_URL` and that Postgres (if used) is actually up; `/health` will
  still return 200 since the process itself is fine.
- **Admin dashboard shows nothing** — sessions are created against whichever
...
- Silero VAD requires the ONNX model file; without it the agent uses an
  adaptive-energy detector with a zero-crossing gate — good enough to keep a
...
- **STT (Deepgram) could not be exercised against the live API in this build
  environment** — outbound network access here is restricted to a small
...
| STT | **PARTIAL** | Real Deepgram provider implemented and unit-tested for its offline path; **not exercised against a live Deepgram account** — this sandbox's network egress does not reach `api.deepgram.com`. Offline fallback verified end-to-end. |
...
| TTS | **PASS** | Reused unmodified; new glue (`app/api/audio.py`) packages it into browser-playable WAV, verified — real audio bytes returned and observed in the E2E run. |
...
| Session State | **PASS** | In-process registry (required — live orchestrators can't be serialized) with an optional Redis mirror of status for multi-replica visibility; degrades cleanly without Redis. |
...
| Docker | **PARTIAL** | `Dockerfile.api`, `frontend/Dockerfile`, `docker-compose.yml` written, `docker-compose.yml` YAML-validated, both Dockerfiles follow the existing project's multi-stage pattern. **Not build-verified** — no Docker daemon is available in this sandbox. Recommend running `docker compose up --build` once and reporting any image-build issue; the compose file and both Dockerfiles are standard enough that I'd expect them to work, but "expect" is not "verified" and this list is for the latter. |
...
missing implementation — and the one item marked BLOCKED, which was blocked
