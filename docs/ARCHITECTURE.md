...
flowchart TB
    ...
    subgraph TRANSPORT["Transport â€” LiveKit WebRTC"]
    ...
    subgraph WORKER["Screening worker (single process)"]
        ...
        ORCH["Screening orchestrator<br/>speculation Â· turn accounting"]
        FLOW["Interview flow<br/>stage machine Â· probe selection"]
        SYNTH["Streaming synthesis<br/>clause chunking Â· abortable"]
        SCRUB["PII scrubber<br/>single-pass Â· offset-traceable"]
    end

    subgraph RETRIEVAL["Retrieval â€” hot path"]
        MOSS["Moss runtime<br/>in-process Â· no vector DB<br/>36 competency documents"]
        FALLBACK["Embedded index<br/>exact hybrid Â· fallback"]
    end

    subgraph COLD["Cold path â€” off the conversational loop"]
        COG["Streaming cognition<br/>temperature 0.2 Â· 150 tok"]
...
sequenceDiagram
    ...
    Note over V,S: Speculation window â€” paid for by silence the candidate is producing anyway
    ...
    alt Candidate resumes talking
        ...
        O->>L: abort â€” draft discarded, cost is compute only
    else Silence holds
...
With it, **12.5ms** â€” a 71% reduction, asserted directly by
...
```mermaid
flowchart LR
    subgraph T0["Agent speaking â€” epoch 0"]
    ...
    subgraph T1["Candidate interrupts"]
        D["1. drain() â†’ epoch := 1<br/>2. synthesiser.abort()<br/>3. transport.clear_egress()"]
    end

    subgraph T2["After â€” epoch 1"]
...
flowchart TB
    ...
    WHOLE --> PROB["Answer matches 1 of 4<br/>competencies; averaged<br/>against 3 irrelevant ones<br/>â†’ signal buried"]
    PER --> GOOD["Tight documents.<br/>Returns WHICH competency<br/>matched â€” the thing the<br/>interviewer needs"]

    PER --> MOSS["Moss index<br/>hybrid semantic + keyword"]
    MOSS --> Q["query(evidence, top_k=9)"]
    Q --> COLLAPSE["Collapse: best competency per role"]
    COLLAPSE --> CTX["Interviewer context<br/>'reads as evidence for: â€¦'"]
...
Over-fetching (`top_k = 3 Ã— limit`) then collapsing per role is deliberate:
...
flowchart LR
    ...
    EV --> DET["Deterministic matcher<br/>IDF-weighted signals<br/>reproducible Â· auditable"]
...
inflate the candidate's score â€” the classic self-confirming evaluation bug.
...
```mermaid
flowchart LR
    IN["Resume Â· live transcript"] --> S["Single-pass scrubber"]

    S --> S1["1. Scan all patterns<br/>against ORIGINAL text"]
    S1 --> S2["2. Reject matches inside<br/>existing tokens<br/>â†’ structural idempotence"]
